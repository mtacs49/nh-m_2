...
def test():
    ...
    print("\nECDSA private key (random number / secret exponent) = {}".format(hex_private_key))
    print("\nECDSA public key = {}".format(''.join(['{:02x}'.format(i) for i in public_key])))
    #print("\nBitcoin private key (Base58Check) = {}".format(base58_check(private_key, version=128)))
 
    addr = generate_address(public_key)
    print("\nBitcoin Address: {}".format(addr))
